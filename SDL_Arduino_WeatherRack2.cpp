//
//   SDL_Arduino_WeatherRack2 Library
//   SDL_Arduino_WeatherRack2.cpp
//   Version 1.2
//   SwitchDoc Labs   September 2020
//   Supports WeatherSense WeatherRack2 and Indoor T/H Sensor from SwitchDoc Labs
//
/*
  ...
  Modified by SwitchDoc Labs to support WeatherSense WeatherRack2 and SDL Indoor TH sensor.


  Based on Andrew's sketch for capturing data from F016TH Thermo-Hygrometer and with the addition of a checksum check.
  ...
  What this code does:
  Captures F016TH (and now TF0300) Thermo-Hygrometer data packets by;
  ...
  The F0300 sends about 9 additional bytes of data for the WeatherRack2 and is formatted differently but has a similar structure

*/


// Prints intermediate steps for debugging - will affect the timing and reduce good messages
#undef WR2DEBUG

// Designed for Logic Analyzer debugging uses a series of pins to detect what is happening.  Should not affect good messages
#define WRLOGIC
...
int messageFoundPin = 11;
int PinHeaderTest = 9;
int pinHeaderValue = 0;
int PinHeaderBitTest = 13;
int pinHeaderBitValue = 0;
#endif

// Class Functions

SDL_Arduino_WeatherRack2::SDL_Arduino_WeatherRack2(long timeout, boolean read_weatherrack2 , boolean read_indoorth   )
{
  _timeout = timeout;
...
void SDL_Arduino_WeatherRack2::begin()
{
...
int SDL_Arduino_WeatherRack2::waitForNextMessage()
{
  int returnMessage;

  returnMessage = returnMessageValues();
  return returnMessage;


}

void SDL_Arduino_WeatherRack2::setTimeout(long my_timeout)
{
  _timeout = my_timeout;
}

void SDL_Arduino_WeatherRack2::set_ReadWeatherRack2(boolean my_read_weatherrack2)
{
  _read_weatherrack2 = my_read_weatherrack2;

}

void SDL_Arduino_WeatherRack2::set_ReadIndoorth(boolean my_readindoorth)
{

  _read_indoorth = my_readindoorth;
}

long SDL_Arduino_WeatherRack2::readHeadersFound()
{

  return headersFound;
}

long SDL_Arduino_WeatherRack2::readWeatherRack2Found()
{
  return FT300MessagesFound;

}

long SDL_Arduino_WeatherRack2::readSDLIndoorTHFound()
{
...
byte    headerBits = 8;   //The number of ones expected to make a valid header
...
// Variables to prepare recorded values for devicew

byte stnId = 0;      //Identifies the channel number
int dataType = 0;    //Identifies the  Thermo-Hygrometer code
int Newtemp = 0;
int Newhum = 0;
...
int SDL_Arduino_WeatherRack2::returnMessageValues()
{


  boolean messageFound = false;
  int returnValue = NO_MESSAGE;
  ...
  while ((returnValue == NO_MESSAGE) && (endTime > millis()))
  {
    ...
    maxBytes = MAX_BYTES;  // to short message
    ...
    while (noErrors && (nosBytes < maxBytes))
    {
      ...
      if ((digitalRead(RxPin) != tempBit ) || (noErrors == false))
      {
      ...
      else
      {
        ...
        if (bitState == 1) //1 data could be header or packet
        {
          ...
          {
            returnValue = add(bitState);//already seen first zero so add bit in
          }
        ...
        { //bitState==0 could first error, first zero or packet
          ...
          else
          {
            ...
            {
              firstZero = true;
              returnValue = add(bitState); //Add first zero to bytes
              //Serial.print("!");
            }//end of finding first zero
            else
            {
              returnValue = add(bitState);
...
int SDL_Arduino_WeatherRack2::add(byte bitData)
{
#ifdef WRLOGIC
  ...
  if ((maxBytes != FT300MAXBYTES) && (nosBytes > 1) && (!dataTypeDetected))
  {
  ...
  if (nosBytes == maxBytes)
  {
    ...
    if ((dataType == 0x45) && (Newhum <= 100))
    {
      ...
      // build structure
      currentIndoorTH.messageid = messageID;
      currentIndoorTH.time = "";
      currentIndoorTH.device = device ;
      currentIndoorTH.modelnumber = 5;
      currentIndoorTH.channel = stnId;
      currentIndoorTH.battery = battery;
      currentIndoorTH.temperature = tempc;
      currentIndoorTH.humidity = Newhum;
      currentIndoorTH.CRC = myFT007CalculatedChecksum;

      return MESSAGE_INDOORTH_GOOD;
    ...
    if ((dataType == 0x4C))
    {
      ...
      myCRC = b2[13];
#ifdef WR2DEBUG
Serial.print("->myCalculated=");
Serial.println(myCalculated,HEX);
Serial.print("->myCRC=");
Serial.println(myCRC, HEX);
#endif
      
      myCalculated = myCalculated & 0xF0;   // remove lower order bits due to demodulation loss
      if (myCalculated == myCRC)
      {
        ...
        Serial.print(F("myDevice = ")); Serial.print(myDevice, HEX  ); Serial.print(" "); Serial.println(myDevice);
        Serial.print(F("mySerial = ")); Serial.print(mySerial, HEX  ); Serial.print(" "); Serial.println(mySerial );
        Serial.print(F("myFlags = ")); Serial.print(myFlags, HEX  ); Serial.print(" "); Serial.println(myFlags );
        Serial.print(F("myBatteryLow = ")); Serial.print( myBatteryLow, HEX   ); Serial.print(" "); Serial.println( myBatteryLow  );
        Serial.print(F("myAveWindSpeed = ")); Serial.print( myAveWindSpeed, HEX  ); Serial.print(" "); Serial.println( myAveWindSpeed );
        Serial.print(F("myGust = ")); Serial.print( myGust, HEX  ); Serial.print(" "); Serial.println( myGust );
        Serial.print(F("myWindDirection = ")); Serial.print(  myWindDirection, HEX  ); Serial.print(" "); Serial.println(  myWindDirection  );
        Serial.print(F("myCumulativeRain = ")); Serial.print(  myCumulativeRain, HEX  ); Serial.print(" "); Serial.println( myCumulativeRain  );
        Serial.print(F("mySecondFlags = ")); Serial.print( mySecondFlags, HEX  ); Serial.print(" "); Serial.println( mySecondFlags );
        Serial.print(F("myTemperature = ")); Serial.print( myTemperature, HEX  ); Serial.print(" "); Serial.println( myTemperature);
        Serial.print(F("myHumidity = ")); Serial.print( myHumidity, HEX  ); Serial.print(" "); Serial.println( myHumidity);
        Serial.print(F("myLight = ")); Serial.print( myLight, HEX  ); Serial.print(" "); Serial.println( myLight );
        Serial.print(F("myUV = ")); Serial.print( myUV, HEX  ); Serial.print(" "); Serial.println( myUV );
        Serial.print(F("myCRC = ")); Serial.print(  myCRC, HEX  ); Serial.print(" "); Serial.println(  myCRC);
        Serial.print(F("myCalculated  =")); Serial.print(  myCalculated, HEX  ); Serial.print(" "); Serial.println(  myCalculated  );
        ...
        currentWR2.messageid = messageID;
        currentWR2.time = "";
        currentWR2.device = myDevice;
        currentWR2.modelnumber = mySerial;
        currentWR2.battery = myBatteryLow;
        currentWR2.avewindspeed = myAveWindSpeed;
        currentWR2.gustwindspeed = myGust;
        currentWR2.winddirection = myWindDirection;
        currentWR2.cumulativerain = myCumulativeRain;
        currentWR2.temperature = tempc;
        currentWR2.humidity = myHumidity;
        currentWR2.light = myLight;
        currentWR2.uv = myUV;
        currentWR2.CRC = myCRC;

        return MESSAGE_WEATHERRACK2_GOOD;

      }
      else
      {
        Serial.println("F0300 Bad CRC");
        return MESSAGE_CRC_ERROR_WR2;
      }

    }


  } // end of set to 16 as dataype = 4C

  return NO_MESSAGE;
}

void SDL_Arduino_WeatherRack2::eraseManchester()
{
  for ( int j = 0; j < 4; j++)
  {
    manchester[j] = j;
...
uint8_t SDL_Arduino_WeatherRack2::Checksum(int length, uint8_t *buff)
{
  uint8_t mask = 0x7C;
...
// Return:   CRC value
//============================================================================================
uint8_t SDL_Arduino_WeatherRack2::GetCRC(uint8_t crc, uint8_t * lpBuff, uint8_t ucLen)
{
  while (ucLen)
  {
    ucLen--;
